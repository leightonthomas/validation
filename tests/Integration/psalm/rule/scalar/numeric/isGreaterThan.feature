Feature: IsGreaterThan Rule with no plugin
  In order to write and understand code correctly
  As a user of the library
  I need to be able to see the correct type from the IsGreaterThan rule
  ...
  Scenario Outline: It will return the correct type on construction
    ...
    Then I see these errors
      | Type  | Message                                                                    |
      | Trace | $rule: LeightonThomas\Validation\Rule\Scalar\Numeric\IsGreaterThan<<type>> |
  ...
  Scenario Outline: It will add Psalm issue if non-numeric value given
    Given I have the following code
      """
      new IsGreaterThan(<value>);
      """
    When I run Psalm
    Then I see these errors
      | Type    | Message                                                                                                                 |
      | <error> | Argument 1 of LeightonThomas\Validation\Rule\Scalar\Numeric\IsGreaterThan::__construct expects numeric, <type> provided |
    And I see no other errors
